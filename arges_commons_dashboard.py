...
with st.sidebar:
    st.markdown(f'<div class="brand-mark" style="font-size:24px">Johnson&Johnson</div>', unsafe_allow_html=True)
    st.caption("ARGES Commons • Clinical Data Inventory")
    uploaded = st.file_uploader("Workbook", type=["xlsx"], help="Optional: upload a workbook to override the local ARGES_COMMONS.xlsx file.")
...
with st.sidebar:
    st.markdown("### Filters")
    phases = st.multiselect("Phase", clean_text_values(studies.get("Phase", pd.Series(dtype=str))))
    statuses = st.multiselect("Trial status", clean_text_values(studies.get("Trial Status", pd.Series(dtype=str))))
    compounds = st.multiselect("Compound", clean_text_values(studies.get("Compound", pd.Series(dtype=str))))
    ...
    study_options = clean_text_values(eligible["Study Name"]) if "Study Name" in eligible else []
...
# -----------------------------------------------------------------------------
st.markdown(
    f'<div class="brand"><div class="brand-mark">Johnson&Johnson</div><div class="brand-divider"></div><div class="brand-sub">ARGES Commons</div></div>',
    unsafe_allow_html=True,
)
st.markdown(
...
if page == "Overview":
    ...
    with left:
        ...
        if "Disease" in s and not s.empty:
            ...
            fig.update_layout(height=350, margin=dict(l=5,r=10,t=15,b=10), yaxis_title="", xaxis_title="Studies", showlegend=False)
            st.plotly_chart(fig, use_container_width=True)
        else: st.info("No study data for the current filters.")
    with right:
        donut_from_status(s, "Trial Status", "Trial status")

    st.subheader("Data journey")
    st.markdown('<span class="pill">Studies</span><span class="pill">Assets</span><span class="pill">Processing</span><span class="pill">Splits</span><span class="pill">ARD</span><span class="pill">Variables</span><span class="pill">Analysis</span><span class="pill">External Data</span>', unsafe_allow_html=True)
    st.caption("Use the navigation above to drill into each stage. Global filters apply across all study-linked sections.")

    if not ard.empty and {"Study Name","Coverage %"}.issubset(ard.columns):
        st.subheader("ARD mapping coverage")
        chart = ard[["Study Name","Coverage %"]].copy(); chart["Coverage %"] = pd.to_numeric(chart["Coverage %"], errors="coerce")
        chart = chart.dropna().sort_values("Coverage %")
        fig = px.bar(chart, x="Coverage %", y="Study Name", orientation="h", text_auto=".0%", color_discrete_sequence=[JNJ_RED])
        fig.update_xaxes(tickformat=".0%", range=[0,1]); fig.update_layout(height=max(320, 28*len(chart)), margin=dict(l=5,r=15,t=15,b=10), yaxis_title="")
        st.plotly_chart(fig, use_container_width=True)
