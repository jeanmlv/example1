...
if page == "Overview":
    ...
    st.markdown('<div class="section-note">Study-level portfolio summary from 01_STUDIES. The table responds to the global filters.</div>', unsafe_allow_html=True)
    summary_cols = [
        "Study ID", "Study Name", "Disease", "Pediatric", "Patients", "Videos",
        "Patients w/ Videos", "ClinicalTrials Link"
    ]
    summary_cols = [c for c in summary_cols if c in s.columns]
    summary = s[summary_cols].copy()
    show_table(summary, key="overview_study_summary", max_rows=12, link_cols=["ClinicalTrials Link"])
